...
add_function_renamings(Module, New, Names, Map) ->
    ...
    lists:foldl(
      fun (F = {_, A}, {NamesAcc, MapAcc}) when is_integer(A) ->
	      F1 = new_function_name(Module, F, NamesAcc),
	      {sets:add_element(F1, NamesAcc), dict:store(F, F1, MapAcc)}
      end,
...
add_record_renamings(Module, Records, Names, Map, RMap) ->
    lists:foldl(
      fun (N = {R, Fs}, {NamesAcc, MapAcc, RMapAcc}) ->
	      case sets:is_element(?record_name(R), NamesAcc) of
		  true ->
		      %% The name is already in use.
		      case sets:is_element(?record_name(N), NamesAcc) of
			  true ->
			      %% We have seen this definition before;
			      %% make sure we use the same name.
			      {R1, _} = remap_record_name(N, RMapAcc),
			      Map1 = dict:store(?record_name(R),
						?record_name(R1), MapAcc),
			      {NamesAcc, Map1, RMapAcc};
			  false ->
		  ...
		  false ->
		      ...
		      Names1 = sets:add_element(?record_name(R), NamesAcc),
		      Names2 = sets:add_element(?record_name(N), Names1),
		      {Names2, MapAcc, RMapAcc}
	      end
      end,
...
fold_record_field({Name, {F, Type}}) ->
	...
	false ->
	    ...
	    {Name, {hash, erlang:phash2(F, 16#ffffff)}, Type}
    end;
...
fold_record_field({Name, F}) ->
	...
	false ->
	    ...
	    %% represent it by a hash value instead. (We don't want to
	    %% do this in the general case.)
	    {Name, {hash, erlang:phash2(F, 16#ffffff)}}
    end.
